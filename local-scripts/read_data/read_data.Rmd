...
VC55.weather.sl <- suttonboningtondata %>% 
 ...
 VC55.weather.sl$YYYMMMDD <-   with( VC55.weather.sl, 
                                     paste( YYYY, mm, 
                                            dd, sep= "-"   )    )  
 
 # Reorganise the names into a nice order
 ss.colnames <- c("YYYMMMDD",
                  "datum.attribute",
                  "value"
 )
...
VC55.weather.ss$YYYMMMDD <-   VC55.weather.ss$YYYMMMDD %>% as.Date()
...
ggplot(data = VC55.weather.ss, aes(colour = factor(datum.attribute), x= YYYMMMDD, y =value, group = datum.attribute) ) + 
  geom_point() +
    geom_line()
