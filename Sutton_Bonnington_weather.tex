...
\NormalTok{ suttonboningtondata}\SpecialCharTok{$}\NormalTok{YYYYMMDD }\OtherTok{\textless{}{-}}\NormalTok{ timeDate}\SpecialCharTok{::}\FunctionTok{timeLastDayInMonth}\NormalTok{(dummyDate) }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{as.character}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{  path.to.data,}
  \AttributeTok{pattern =} \StringTok{".rds$"}\NormalTok{, }\CommentTok{\# Make a suitable filter. Use the dot for a wildcard. note the use of the $ to match the files extension.}
...
\NormalTok{weather.attribute.list }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}
 \StringTok{"max temp degC"} \OtherTok{=} \StringTok{"tmax.degC"}\NormalTok{,}
 \StringTok{"min temp degC"} \OtherTok{=} \StringTok{"tmin.degC"}\NormalTok{,}
\StringTok{"airfrost days"} \OtherTok{=}\StringTok{"airfrost.days"}\NormalTok{,}
\StringTok{"rain mm"} \OtherTok{=} \StringTok{"rain.mm"}
\NormalTok{) }

\NormalTok{title.list }\OtherTok{\textless{}{-}}\NormalTok{ weather.attribute.list }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{as.data.frame}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}\NormalTok{ row.names}
  
\NormalTok{weather.plot }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(weather.attribute, data.to.use, nice.title ) \{}
  

\NormalTok{data.to.plot }\OtherTok{\textless{}{-}}\NormalTok{ data.to.use[data.to.use}\SpecialCharTok{$}\NormalTok{datumAttribute }\SpecialCharTok{==}\NormalTok{ weather.attribute, ]}
...
\NormalTok{plot.output }\OtherTok{\textless{}{-}}    \FunctionTok{ggplot}\NormalTok{(}\AttributeTok{data =}\NormalTok{data.to.plot.annual, }\FunctionTok{aes}\NormalTok{( }\AttributeTok{x=}\NormalTok{ YYYY, }\AttributeTok{y =}\NormalTok{annual) ) }\SpecialCharTok{+} 
  \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+} 
   \FunctionTok{geom\_smooth}\NormalTok{( }\AttributeTok{se =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{method =} \StringTok{"lm"}\NormalTok{, }\AttributeTok{formula =} \StringTok{"y \textasciitilde{} x"}\NormalTok{) }\SpecialCharTok{+}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{weather.plots.list}\SpecialCharTok{$}\StringTok{\textasciigrave{}}\AttributeTok{max temp degC}\StringTok{\textasciigrave{}}
\end{Highlighting}
\end{Shaded}
...
\leavevmode\vadjust pre{\hypertarget{ref-RStudioTeam2016}{}}%
